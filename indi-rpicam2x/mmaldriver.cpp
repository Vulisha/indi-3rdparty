...
VCOS_LOG_CAT_T indi_rpicam2x_log_category;

MMALDriver::MMALDriver() : INDI::CCD(), chipWrapper(&PrimaryCCD)
{
    LOGF_DEBUG("%s()", __FUNCTION__);
    setVersion(indi_rpicam2x_VERSION_MAJOR, indi_rpicam2x_VERSION_MINOR);

    bcm_host_init();

    // Register our application with the logging system
    vcos_log_register("indi_rpicam2x", &indi_rpicam2x_log_category);
...
void MMALDriver::addFITSKeywords(INDI::CCDChip * targetChip)
{
    ...
    if (mIsoSP.nsp > 0)
    {
        ...
        if (onISO)
        {
            ...
            if (isoSpeed > 0)
            {
                fits_update_key_s(TUINT, "ISOSPEED", &isoSpeed, "ISO Speed", &status);
            }
        }
    }
#endif
